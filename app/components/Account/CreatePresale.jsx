...
class AccountPresaleCreate extends React.Component {
    ...
    onAmountChanged({amount, asset}) {console.log(asset)
    ...
    _checkAssets(assets, force) {
        ...
        let myAssets = assets.filter(asset => {
            return asset.issuer === this.props.account.get("id");
        })
        console.log(myAssets.toJS())
        if (Object.keys(myAssets.toJS()).length)
            this.setState({asset: ChainStore.getObject(Object.keys(myAssets.toJS())[0])});
    }
    ...
    render() {
        ...
        let supports = this.props.assets.filter(ast => {
            return ast.id != this.state.asset.id;
        }).map(ast => {
        ...
        let myAssetIds = Object.keys(myAssets);console.log(this.state.asset,myAssetIds)
            ...
            <div className="grid-block" style={{paddingTop: "30px"}}>
                <div className="grid-content large-12 small-12">
                    ...
                    <AmountSelector
                        ...
                        asset={this.state.asset ? this.state.asset.get("id") : myAssetIds[0]}
                        assets={myAssetIds}
